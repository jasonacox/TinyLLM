...
if WEAVIATE_HOST != "":
    ...
    try:
        ...
        log(f"Unable to connect to Weaviate at {WEAVIATE_HOST}: {str(e)}")


# Find document closely related to query
def query_index(query, library, num_results=2):
...
async def handle_normal_prompt(session_id, p):
    ...
    log("------> %s" % p)
    if WEAVIATE_HOST and ONESHOT and p is not prompts["greeting"]:
